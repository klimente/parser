...
"""def get_html(html):                     #парс названий матчей
    soup = BeautifulSoup(html,'lxml').find('div',id='block-system-main').find_all('div',class_='black-title-content-border')  #создаем объект супа, сразу вызываем методы find и find_all , который возвращает свой список
    for p in soup:     #собирает название всех кубков
        g = p.find('img').get('alt') #для каждого img из списка супа  достаем атрибут alt
        m = p.find_all('div',class_ = 'match')
        for i in m:         # собирает все название команд
            d = i.find('td',class_='col-2').find_all('td', class_='team-name')
            team = []
            for y in d:
                o = y.find('a').string
                team.append(o)
...
def get_results_h(html):                              #Ф-ция , в которой достаю все необходиые данные для анализа игры
    ...
    return games_at_home,games_visit,home_goals,visit_goals, average_goals_h,average_goals_missed
...
def get_results_v(html):                              #Ф-ция , в которой достаю все необходиые данные для анализа игры
    ...
    return games_at_home, games_visit,home_goals,visit_goals,average_goals_v,average_goals_missed_v,
